...
router.get('/editorial/:idpublisher', function (req, res) {
    res.header('Access-Control-Allow-Origin', '*');
    Libro.find({ IDpublisher: req.params.idpublisher}, (err, libros) =>{
        if (err) return res.status(500).send("Problemas buscando");
        if (!libros) return res.status(404).send("Libros no encontrados para editorial: " + req.params.idpublisher);
        res.status(200).send(libros);
    });
});
